...
import { useCreateProductUnit, useDeleteProductUnitById, useEditProductUnit, useGetProductUnit } from '../../../../ApiHooks/OtherSetting/ProductUnit';
const ProductUnit = () => {
  ...
  const refetchSourceRef = async () => {
    const result = await refetchProductUnitList();
    if (result?.data) {
  ...
  const { mutateAsync: CreateProductUnit } = useCreateProductUnit();
  const { mutateAsync: UpdateProductUnit } = useEditProductUnit();
  const { mutateAsync: DeleteProductUnit } = useDeleteProductUnitById();
  const { data: ProductUnitList, refetch: refetchProductUnitList, isPending } = useGetProductUnit();
  const { control, setValue, getValues } = useForm();
  useEffect(() => {
    if (ProductUnitList) {
      const sortedData = ProductUnitList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const handleSaveProductUnit = async (record) => {
    ...
    try {
      if (record.isNew) {
        ...
        await UpdateProductUnit(updatedProductUnit);
        ...
        if (editingKey === record.Id || record.isNew) {
          return (
            <div size='middle ' className='gap-3 flex justify-center content-center'>
              <div>
                <FiCheck className='size-5' onClick={() => handleSaveProductUnit(record)} />
              </div>
