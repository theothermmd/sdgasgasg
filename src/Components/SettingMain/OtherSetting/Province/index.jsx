...
import { useCreateProvince, useDeleteProvinceById, useEditProvince, useGetProvince } from '../../../../ApiHooks/OtherSetting/Province';
...
const Province = () => {
  ...
  const refetchSourceRef = async () => {
    const result = await refetchProvinceList();
    if (result?.data) {
      const sortedData = result.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const { mutateAsync: UpdateProvince } = useEditProvince();
  const { mutateAsync: DeleteProvince } = useDeleteProvinceById();
  const { data: ProvinceList, refetch: refetchProvinceList, isPending } = useGetProvince();
  ...
  useEffect(() => {
    ...
    const transformedData = transformData(ProvinceList?.data);
  ...
  const handleSave = async (record) => {
    ...
    try {
      if (record.isNew || record.isChild) {
        ...
        const { data } = await CreateProvince(newData);
        if (data.isSuccess) {
          toast.success('مورد مدنظر با موفقیت اضافه شد');
        } else {
        ...
        const { data } = await UpdateProvince(updatedData);
        if (data.isSuccess) {
      ...
      setChecked(false);
      refetchProvinceList(); // بازخوانی داده‌ها
    } catch (error) {
