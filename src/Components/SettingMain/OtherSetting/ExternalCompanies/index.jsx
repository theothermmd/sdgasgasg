...
import { useCreateExternalCompanies, useDeleteExternalCompaniesById, useEditExternalCompanies, useGetExternalCompanies } from '../../../../ApiHooks/OtherSetting/ExternalCompanies';
const ExternalCompanies = () => {
  ...
  const refetchSourceRef = async () => {
    const result = await refetchExternalCompaniesList();
    if (result?.data) {
  ...
  const { mutateAsync: CreateExternalCompanies } = useCreateExternalCompanies();
  const { mutateAsync: UpdateExternalCompanies } = useEditExternalCompanies();
  const { mutateAsync: DeleteExternalCompanies } = useDeleteExternalCompaniesById();
  const { data: ExternalCompaniesList, refetch: refetchExternalCompaniesList, isPending } = useGetExternalCompanies();
  const { control, setValue, getValues } = useForm();
  useEffect(() => {
    if (ExternalCompaniesList) {
      const sortedData = ExternalCompaniesList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const handleSaveExternalCompanies = async (record) => {
    ...
    try {
      if (record.isNew) {
        ...
        const { data } = await CreateExternalCompanies(newExternalCompanies);
        if (data.isSuccess) {
        ...
        const updatedExternalCompanies = {
          Id: record.Id,
          Title,
          Phone,
          Fax,
        ...
        const { data } = await UpdateExternalCompanies(updatedExternalCompanies);
        if (data.isSuccess) {
          toast.success('مورد مد نظر با موفقیت ویرایش شد');
        } else {
          toast.error(data.message);
        }
  ...
  const column = [
    ...
    },
    {
      ...
      key: 'Fax',
      align: 'center',
      sortOrder: sortedInfo.columnKey === 'Fax' ? sortedInfo.order : null,
      ellipsis: true,
        ...
        if (editingKey === record.Id || record.isNew) {
          return (
            <div size='middle ' className='gap-3 flex justify-center content-center'>
              <div>
                <FiCheck className='size-5' onClick={() => handleSaveExternalCompanies(record)} />
              </div>
    ...
    swalWithTailwindButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          try {
            await DeleteExternalCompanies(id);
            refetchSourceRef();
