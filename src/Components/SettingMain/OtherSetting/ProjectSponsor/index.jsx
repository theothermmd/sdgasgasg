...
import { useCreateProjectSponsor, useDeleteProjectSponsorById, useEditProjectSponsor, useGetProjectSponsor } from '../../../../ApiHooks/OtherSetting/ProjectSponsor';
...
const ProjectSponsor = () => {
  ...
  const { mutateAsync: Create } = useCreateProjectSponsor();
  const { mutateAsync: Update } = useEditProjectSponsor();
  const { mutateAsync: Delete } = useDeleteProjectSponsorById();
  const { data: ProjectSponsorList, refetch: refetchSourceRef } = useGetProjectSponsor();
  useEffect(() => {
    if (ProjectSponsorList) {
      const sortedData = ProjectSponsorList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const handleSave = async (record) => {
    ...
    try {
      if (record.isNew) {
        ...
        if (data.isSuccess) {
          toast.success('حامی پروژه با موفقیت اضافه شد');
        } else {
        ...
        if (data.isSuccess) {
          toast.success('حامی پروژه با موفقیت ویرایش شد');
        } else {
          toast.error(data.message);
        }
      }
      ...
      toast.error('خطا در ذخیره حامی پروژه');
    }
  };
  const column = [
    {
