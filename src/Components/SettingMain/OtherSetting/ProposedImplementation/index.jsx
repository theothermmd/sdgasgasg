...
import {
  useCreateProposedImplementation,
  useDeleteProposedImplementationById,
  useEditProposedImplementation,
  useGetProposedImplementation,
} from '../../../../ApiHooks/OtherSetting/ProposedImplementation';
const ProposedImplementation = () => {
  ...
  const { mutateAsync: Create } = useCreateProposedImplementation();
  const { mutateAsync: Update } = useEditProposedImplementation();
  const { mutateAsync: Delete } = useDeleteProposedImplementationById();
  const { data: ProposedList, refetch: refetchSourceRef } = useGetProposedImplementation();
  useEffect(() => {
    if (ProposedList) {
      const sortedData = ProposedList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const handleSaveProposed = async (record) => {
    ...
    try {
      if (record.isNew) {
        ...
        const { data } = await Update(updatedData);
        if (data.isSuccess) {
      ...
      toast.error('خطا در ذخیره واحد پولی');
    }
  };
        ...
        if (editingKey === record.Id || record.isNew) {
          return (
            <div size='middle ' className='gap-3 flex justify-center content-center'>
              <div>
                <FiCheck className='size-5' onClick={() => handleSaveProposed(record)} />
              </div>
    ...
    <>
      <div className=' border-b flex gap-4'>
        <div onClick={handleAdd} className=' flex gap-2 text-primary pb-3 cursor-pointer'>
          ...
          <div style={{ fontSize: '13px' }} className='font-bold text-primary'>
            روش اجرای پیشنهادی{' '}
          </div>
        </div>
      </div>
...
export default ProposedImplementation;
