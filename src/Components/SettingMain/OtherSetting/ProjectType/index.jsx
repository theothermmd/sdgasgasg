...
import { SearchOutlined, ReconciliationOutlined } from '@ant-design/icons';
import Swal from 'sweetalert2';
import { useCreateProjectType, useDeleteProjectTypeById, useEditProjectType, useGetProjectType } from '../../../../ApiHooks/OtherSetting/ProjectType';
import toast, { Toaster } from 'react-hot-toast';
import CustomModal from './../../../CustomModal';
import CheckList from './CheckList';
import { name } from 'react-date-object/calendars/julian';
import { useDispatch, useSelector } from 'react-redux';
import { handleAddId, handleShowModal, handleEntityType } from '../../../../features/Modal/ModalSlice';
...
const ProjectType = () => {
  ...
  const refetchSourceRef = async () => {
    const result = await refetchProjectTypeList();
    if (result?.data) {
  ...
  const { mutateAsync: CreateProjectType } = useCreateProjectType();
  const { mutateAsync: UpdateProjectType } = useEditProjectType();
  const { mutateAsync: DeleteProjectType } = useDeleteProjectTypeById();
  const { data: ProjectTypeList, refetch: refetchProjectTypeList, isPending } = useGetProjectType();
  const { control, setValue, getValues } = useForm();
  useEffect(() => {
    if (ProjectTypeList) {
      const sortedData = ProjectTypeList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const handleSaveProjectType = async (record) => {
    //;
    ...
    try {
      if (record.isNew) {
        ...
        const { data } = await CreateProjectType(newProjectType);
        if (data.isSuccess) {
          toast.success('نوع پروژه با موفقیت اضافه شد');
        } else {
        ...
        const { data } = await UpdateProjectType(updatedProjectType);
        if (data.isSuccess) {
          toast.success('نوع پروژه با موفقیت ویرایش شد');
        } else {
          toast.error(data.message);
        }
        toast.success('نوع پروژه با موفقیت ویرایش شد');
      ...
      toast.error('خطا در ذخیره نوع پروژه');
    }
  };
  ...
  const AddCheckList = ({ record, name }) => {
    dispatch(handleShowModal(name));
    dispatch(handleAddId(record.Id));
    console.log(record, 'ndasndandasdkaskdjasdjaskjakdjkajdkjdkjdkjakd');
  };
        ...
        if (editingKey === record.Id || record.isNew) {
          return (
            <div size='middle ' className='gap-3 flex justify-center content-center'>
              <div>
                <FiCheck className='size-5' onClick={() => handleSaveProjectType(record)} />
              </div>
        ...
        return (
          <div size='middle ' className='gap-3 flex justify-center content-center'>
            <>
              ...
              <Tooltip title='چک لیست ها'>
                <Button type='link' icon={<ReconciliationOutlined style={{ color: '#5b8c00' }} />} onClick={() => AddCheckList({ record, name: 'CheckList' })} />
              </Tooltip>
            </>
          </div>
        );
  ...
  return (
    <>
      ...
      {statusModal === 'CheckList' && <CustomModal Component={<CheckList />} width={'70%'} title='چک لیست:' />}
      <Table
