...
import { useCreateProjectExitType, useDeleteProjectExitTypeById, useEditProjectExitType, useGetProjectExitType } from '../../../../ApiHooks/OtherSetting/ProjectExitType';

const ProjectExitType = () => {
  ...
  const { mutateAsync: CreateProjectExitType } = useCreateProjectExitType();
  const { mutateAsync: UpdateProjectExitType } = useEditProjectExitType();
  const { mutateAsync: DeleteProjectExitType } = useDeleteProjectExitTypeById();
  const { data: ProjectExitTypeList, refetch: refetchProjectExitTypeList, isPending } = useGetProjectExitType();

  const refetchSourceRef = async () => {
    const result = await refetchProjectExitTypeList();
    if (result?.data) {
  ...
  useEffect(() => {
    if (ProjectExitTypeList) {
      const sortedData = ProjectExitTypeList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const handleSaveProjectExitType = async (record) => {
    ...
    try {
      if (record.isNew) {
        const newProjectExitType = {
          Title,
        };
        const { data } = await CreateProjectExitType(newProjectExitType);
        if (data.isSuccess) {
        ...
        const updatedProjectExitType = {
          Id: record.Id,
          Title,
        };
        const { data } = await UpdateProjectExitType(updatedProjectExitType);
        if (data.isSuccess) {
          toast.success('مورد مد نظر با موفقیت بروزرسانی شد');
        } else {
          toast.error(data.message);
        }
      }
        ...
        if (editingKey === record.Id || record.isNew) {
          return (
            <div size='middle ' className='gap-3 flex justify-center content-center'>
              <div>
                <FiCheck className='size-5' onClick={() => handleSaveProjectExitType(record)} />
              </div>
    ...
    swalWithTailwindButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          try {
            await DeleteProjectExitType(id);
            refetchSourceRef();
...
export default ProjectExitType;
