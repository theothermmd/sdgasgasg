...
import { useCreateLocationType, useDeleteLocationTypeById, useEditLocationType, useGetLocationType } from '../../../../ApiHooks/OtherSetting/LocationType';
const LocationType = () => {
  ...
  const { mutateAsync: CreatelocarionType } = useCreateLocationType();
  const { mutateAsync: UpdatelocarionType } = useEditLocationType();
  const { mutateAsync: DeletelocarionType } = useDeleteLocationTypeById();
  const { data: LocationTypeList, refetch: refetchSourceRef } = useGetLocationType();
  useEffect(() => {
    if (LocationTypeList) {
      const sortedData = LocationTypeList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
        ...
        if (editingKey === record.Id || record.isNew) {
          return (
            <div size='middle ' className='gap-3 flex justify-center content-center'>
              <div>
                <FiCheck className='size-5' onClick={() => handleSaveLocationType(record)} />
              </div>
  ...
  const handleSaveLocationType = async (record) => {
    ...
    try {
      if (record.isNew) {
        ...
        const { data } = await CreatelocarionType(newCurrency);
        if (data.isSuccess) {
          toast.success('نوع مکان با موفقیت اضافه شد');
        } else {
        ...
        const { data } = await UpdatelocarionType(updatedCurrency);
        if (data.isSuccess) {
          toast.success('نوع مکان با موفقیت ویرایش شد');
        } else {
          toast.error(data.message);
        }
      }
      ...
      toast.error('خطا در ذخیره نوع مکان');
    }
  };
  ...
  //     try {
  //       if (editingKey !== null) {
  //         const { data } = await UpdateCurrency(updatedCurrency);
  //         if (data.isSuccess) {
