...
import {
  useCreateProjectRevenueForecast,
  useDeleteProjectRevenueForecastById,
  useEditProjectRevenueForecast,
  useGetProjectRevenueForecast,
} from '../../../../ApiHooks/OtherSetting/ProjectRevenueForecast';
const ProjectRevenueForecast = () => {
  ...
  const { mutateAsync: Create } = useCreateProjectRevenueForecast();
  const { mutateAsync: Update } = useEditProjectRevenueForecast();
  const { mutateAsync: Delete } = useDeleteProjectRevenueForecastById();
  const { data: DataList, refetch: refetchSourceRef } = useGetProjectRevenueForecast();
  useEffect(() => {
    if (DataList) {
      const sortedData = DataList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
      ...
      <div className=' border-b flex gap-4'>
        <div onClick={handleAdd} className=' flex gap-2 text-primary pb-3 cursor-pointer'>
          ...
          <div style={{ fontSize: '13px' }} className='font-bold text-primary'>
            پیش بینی و برآورد درآمد پروژه ها{' '}
          </div>
        </div>
      </div>
      <Table
...
export default ProjectRevenueForecast;
