...
import { useCreateCurrency, useDeleteCurrencyById, useEditCurrency, useGetCurrency } from '../../../../ApiHooks/OtherSetting/Currency';
...
const CurrencySelector = () => {
  ...
  const refetchSourceRef = async () => {
    const result = await refetchCurrencyList();
    if (result?.data) {
  ...
  const { mutateAsync: UpdateCurrency } = useEditCurrency();
  const { mutateAsync: DeleteCurrency } = useDeleteCurrencyById();
  const { data: CurrencyList, refetch: refetchCurrencyList, isPending } = useGetCurrency();
  const { control, setValue, getValues } = useForm();
  useEffect(() => {
    if (CurrencyList) {
      const sortedData = CurrencyList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const handleSaveCurrency = async (record) => {
        ...
        if (editingKey === record.Id || record.isNew) {
          return (
            <div size='middle ' className='gap-3 flex justify-center content-center'>
              <div>
                <FiCheck className='size-5' onClick={() => handleSaveCurrency(record)} />
              </div>
