...
import { useCreateProjectGoalUnit, useDeleteProjectGoalUnitById, useEditProjectGoalUnit, useGetProjectGoalUnit } from '../../../../ApiHooks/OtherSetting/ProjectGoalUnit';
const ProjectGoalUnit = () => {
  ...
  const refetchSourceRef = async () => {
    const result = await refetchProjectGoalUnitList();
    if (result?.data) {
  ...
  const { mutateAsync: CreateProjectGoalUnit } = useCreateProjectGoalUnit();
  const { mutateAsync: UpdateProjectGoalUnit } = useEditProjectGoalUnit();
  const { mutateAsync: DeleteProjectGoalUnit } = useDeleteProjectGoalUnitById();
  const { data: ProjectGoalUnitList, refetch: refetchProjectGoalUnitList, isPending } = useGetProjectGoalUnit();
  const { control, setValue, getValues } = useForm();
  useEffect(() => {
    if (ProjectGoalUnitList) {
      const sortedData = ProjectGoalUnitList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
  ...
  const handleSaveProjectGoalUnit = async (record) => {
    ...
    try {
      if (record.isNew) {
        const newProjectGoalUnit = {
          Title,
        };
        const { data } = await CreateProjectGoalUnit(newProjectGoalUnit);
        if (data.isSuccess) {
        ...
        const updatedProjectGoalUnit = {
          Id: record.Id,
          Title,
        };
        const { data } = await UpdateProjectGoalUnit(updatedProjectGoalUnit);
        if (data.isSuccess) {
        ...
        if (editingKey === record.Id || record.isNew) {
          return (
            <div size='middle ' className='gap-3 flex justify-center content-center'>
              <div>
                <FiCheck className='size-5' onClick={() => handleSaveProjectGoalUnit(record)} />
              </div>
    ...
    swalWithTailwindButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          try {
            await DeleteProjectGoalUnit(id);
            refetchSourceRef();
...
export default ProjectGoalUnit;
