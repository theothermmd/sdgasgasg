...
import { useCreateRiskCategory, useDeleteRiskCategoryById, useEditRiskCategory, useGetRiskCategory } from '../../../../ApiHooks/OtherSetting/RiskCategory';
...
const RiskCategory = () => {
  ...
  const { mutateAsync: Create } = useCreateRiskCategory();
  const { mutateAsync: Update } = useEditRiskCategory();
  const { mutateAsync: Delete } = useDeleteRiskCategoryById();
  const { data: ProjectPortfolioList, refetch: refetchProjectPortfolioList, isPending } = useGetRiskCategory();
  ...
  const handleSaveProjectPortfolio = async (record) => {
    ...
    try {
      if (record.isNew || record.isChild) {
        ...
        const { data } = await Create(newProjectPortfolio);
        if (data.isSuccess) {
        ...
        const { data } = await Update(updatedProjectPortfolio);
        if (data.isSuccess) {
