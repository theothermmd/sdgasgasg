...
import { useCreateInterestScale, useDeleteInterestScaleById, useEditInterestScale, useGetInterestScale } from '../../../../ApiHooks/OtherSetting/InterestScale';
...
const InterestScale = () => {
  ...
  const { mutateAsync: Create } = useCreateInterestScale();
  const { mutateAsync: Update } = useEditInterestScale();
  const { mutateAsync: Delete } = useDeleteInterestScaleById();
  const { data: InterestScaleList, refetch: refetchSourceRef } = useGetInterestScale();
  useEffect(() => {
    if (InterestScaleList) {
      const sortedData = InterestScaleList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
