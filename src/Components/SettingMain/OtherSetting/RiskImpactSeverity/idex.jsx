...
import {
  useCreateRiskImpactSeverity,
  useDeleteRiskImpactSeverityById,
  useEditRiskImpactSeverity,
  useGetRiskImpactSeverity,
} from '../../../../ApiHooks/OtherSetting/RiskImpactSeverity';
import toast, { Toaster } from 'react-hot-toast';
const RiskImpactSeverity = () => {
  ...
  const { mutateAsync: Create } = useCreateRiskImpactSeverity();
  const { mutateAsync: Update } = useEditRiskImpactSeverity();
  const { mutateAsync: Delete } = useDeleteRiskImpactSeverityById();
  const { data: RiskList, refetch: refetchSourceRef } = useGetRiskImpactSeverity();
  useEffect(() => {
    if (RiskList) {
      const sortedData = RiskList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
...
export default RiskImpactSeverity;
