...
import { useCreateConnectionType, useDeleteConnectionTypeById, useEditConnectionType, useGetConnectionType } from '../../../../ApiHooks/OtherSetting/ConnectionType';
const ConnectionType = () => {
  ...
  const { mutateAsync: Create } = useCreateConnectionType();
  const { mutateAsync: Update } = useEditConnectionType();
  const { mutateAsync: Delete } = useDeleteConnectionTypeById();
  const { data: ConnectionTypeList, refetch: refetchSourceRef } = useGetConnectionType();
  useEffect(() => {
    if (ConnectionTypeList) {
      const sortedData = ConnectionTypeList?.data.map((item, index) => ({
        ...item,
        key: item.Id,
        Id: item.Id,
        Title: item.Title,
