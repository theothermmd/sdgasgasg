...
const Risks = () => {
  ...
  // Helper function to remove commas for API submission
  ...
  const handleAdd = async (data) => {
    ...
    const newData = {
      ...
      CostImpact: removeCommas(data?.CostImpact),
      CurrencyId: data?.Currency?.value,
      PredictTimeOccurrence: date,
      ExchangeRate: removeCommas(data?.CurrencyExchangeRate),
    };
  ...
  const handleFillData = async (data) => {
    reset({
      ...
      Probability: data?.Probability,
      CostImpact: formatNumberWithCommas(data?.CostImpact),
      Currency: { labell: data?.Currency, value: data?.CurrencyId },
      PredictTimeOccurrence: data?.PredictTimeOccurrence,
      CurrencyExchangeRate: formatNumberWithCommas(data?.ExchangeRate),
    });
    setInEditMode(true);
  ...
  const TransformToTableData = (array) => {
    const newArray = array?.map((item) => ({
      ...
      CostImpact: formatNumberWithCommas(item?.CostImpact),
      ...
      ExchangeRate: formatNumberWithCommas(item?.ExchangeRate),
    }));
    return newArray;
  };
    ...
    <div className='bg-white shadow rounded-2xl p-8'>
      <form onSubmit={handleSubmit(inEditMode ? handleEdit : handleAdd)} className='space-y-6'>
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  {...field}
                  onChange={(e) => {
                    const formatted = formatNumberWithCommas(e.target.value.replace(/,/g, ''));
        ...
        <div className='flex justify-end pt-6'>
          <button type='submit' className='bg-blue-600 text-white px-6 py-2 rounded-full text-sm hover:bg-blue-700 transition' hidden={!IsEdit && IsEdit !== undefined}>
