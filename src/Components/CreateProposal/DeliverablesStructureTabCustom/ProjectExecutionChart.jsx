...
import {
  useGetProjectExecutionChart,
  useCreateProjectExecutionChart,
  useDeleteProjectExecutionChartById,
  useEditProjectExecutionChart,
} from '../../../ApiHooks/ProjectExecutionChart';
...
const ProjectExecutionChart = () => {
  ...
  //#endregion
  //#region hooks
  const { data: ProjectExecutionChartList, refetch: refetchSourceRef } = useGetProjectExecutionChart(projectchartId);
  const { data: UsersList } = useGetUsers();
  const { mutateAsync: Create } = useCreateProjectExecutionChart();
  const { mutateAsync: Update } = useEditProjectExecutionChart();
  const { mutateAsync: Delete } = useDeleteProjectExecutionChartById();
  //#endregion
  //#region functions
  ...
  const handleFillData = async (data) => {
    reset({
      Name: { label: data?.Name, value: data?.NameId },
      position: data?.Position,
    });
    setInEditMode(true);
  ...
  const TransformToTableData = (array) => {
    const newArray = array?.map((item) => ({
      Id: item.Id,
      Name: usersData?.find((user) => user.value === item.UserId)?.label,
      NameId: item.UserId,
  ...
  // #region useEffect
  useEffect(() => {
    if (ProjectExecutionChartList?.data && usersData.length > 0) {
      setDataSource(TransformToTableData(ProjectExecutionChartList?.data));
    }
  }, [ProjectExecutionChartList?.data, usersData]);
  useEffect(() => {
    if (UsersList?.data) {
    ...
    <div className='bg-white shadow rounded-xl p-6'>
      <form onSubmit={handleSubmit(inEditMode ? handleEdit : handleAdd)} className='max-w-6xl  p-6 bg-white rounded-xl'>
        <div className='grid grid-cols-1 md:grid-cols-7 gap-4 items-end'>
          ...
          {/*سمت*/}
          <div className='md:col-span-2'>
            <label className='block mb-1 text-sm font-medium text-gray-700'>سمت </label>
            <Controller name='position' control={control} render={({ field }) => <Input {...field} placeholder='سمت را وارد کنید..' className='rounded-full w-full' />} />
          </div>
...
export default ProjectExecutionChart;
